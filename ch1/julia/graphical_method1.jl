...
begin
	...
	using CairoMakie, FreeTypeAbstraction
	fontsize_theme = Theme(fontsize = 18, font="CMU Serif")
	set_theme!(fontsize_theme)
	
end

# ╔═╡ 6aaeabf3-af27-4db6-a220-257704d920a4
using Latexify
...
begin
	...
	fig1 = Figure(resolution = (600,400), font = "CMU Serif")
	ax = CairoMakie.Axis(fig1[1,1], xlabel = L"x", ylabel = L"f(x)", aspect = AxisAspect(1.5))
...
begin
	...
	fig2 = Figure(resolution = (600,800), font = "CMU Serif")
	bx = Axis3(fig2[1, 1], xlabel=L"x_1", ylabel=L"x_2", zlabel=L"f(x_1,x_2)", aspect = (1.5, 1.5, 1), 	perspectiveness = pe, elevation = π / el, azimuth = az*π,)
	#bx = CairoMakie.Axis3(fig2[1,1], xlabel = L"x", ylabel = L"f(x)", zlabel=L"z")
	#limits!(bx, 0,2,-0.1, 1.0)

	
	xw = 8:-0.2:2; yw = 1:0.2:8;
	zw = [f2(x1,x2) for x1 in xw, x2 in yw]
	
	surface!(bx, xs, ys, zs, colormap=:jet, transparency=true)
	wireframe!(bx, xw, yw, zw, color=:black, linewidth=0.4)
...
$\begin{align*}
	...
	\text{s.t.} \qquad & \qquad \frac{x_1^2}{4} + x_2 - 1 \leq 0 \\
...
begin 
	...
	fig3 = Figure(resolution = (600,400), font = "CMU Serif")
	ax3 = CairoMakie.Axis(fig3[1,1], xlabel = L"x_1", ylabel = L"x_2", aspect = AxisAspect(1.5))
	limits!(ax3, -3,1,-2, 2.0)

	xs3 = -3:0.01:1; ys3 = -3:0.01:3
	la = collect(-1:-0.1:-2.0)
	lb = [-0.2005, -0.5, -0.8]
	lc = [lb; la]
	contour!(ax3, xs3, ys3, f3, levels = lc, label="objective function contours")
	contour!(ax3, xs3, ys3, g1, levels= [0], linewidth=2, color=:red)
	contour!(ax3, xs3, ys3, g2, levels= [0], linewidth=2, color=:green)

	# band g1 -> x1^2/4 + x2 - 1 = 0  
	# x2 = -x1^2/4 + 1
	g1_up = x -> -x^2/4 + 1
	g2_low = x -> 1 + x/2
	xs3_bound = -2:0.1:0
	
	band!(ax3, xs3_bound, g1_up.(xs3_bound), g2_low.(xs3_bound), color=(:cyan, 0.7), label = "Feasible region")
...
# ╔═╡ b95eff31-8b2e-44ee-b4ad-b8de29206478
save("/mnt/e/OneDrive/Public/workKMUTT/INC Selection Optimization/Lecture2022/images/graphical_1.png", fig3)

# ╔═╡ 220e3dc7-2d2b-42e0-8843-60fec84f2126
save("/mnt/e/OneDrive/Public/workKMUTT/INC Selection Optimization/Lecture2022/images/graphical_1.pdf", fig3)
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "c7a0c8dea3aa08189de2d68d8a956fb821afcddd"
